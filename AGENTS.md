...
- **External Dependencies:**
  - `curl` (likely used for HTTP requests via `plenary` or built-in `vim.system` - _Note: The code uses `ai.utils.requests` which likely wraps `curl` or a Lua HTTP client_)
...
- **Run API integration tests:**
  ...
  ```
  When debugging, you MUST focus your tests and activate debug logging for more insights: `just test-api --only openai_responses --debug`
